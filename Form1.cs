...
namespace Multiplayer_game_met_bois
{
    public partial class Form1 : Form   //Server class
    {
        ...
        private void TimerUpdate(object sender, EventArgs e)   //60 keer per sekonde
        {
            ...
            Graphics g;
            int PanForce = (int)(tank.force.x * 6);
            ...
            if ((int)(tank.force.x * 6) != 0) //&& 400 < tank.position.X && tank.position.X < 3600
            {
                ...
                g.Clear(Color.Black); 
                bitmap = terrain.TerrainImage(bitmap);   //hierdie veroorsaak bugs
            ...
            if (Client.connected)   //As hy die client is gebeur die
            {
                ...
                g = Graphics.FromImage(bitmap);
                g.DrawRectangle(Pens.Black, ServerTank.position.X + 1, ServerTank.position.Y, 10, 10);
                g.FillRectangle(Brushes.Black, ServerTank.position.X + 1, ServerTank.position.Y, 10, 10);
                ServerTank.position = new Point(x + 1 + PanForce, y);   //PanForce was nie daar nie
                ServerTank.cPosition = new Coordinate(x + 1 + PanForce, y);
                
                g.DrawRectangle(Pens.White, ServerTank.position.X + 1, ServerTank.position.Y, 10, 10);
                g.FillRectangle(Brushes.White, ServerTank.position.X + 1, ServerTank.position.Y, 10, 10);
                pictureBox1.Image = bitmap;            
            } 
            if (Server.ClientTankCords!= "") //As hy die server is gebeur die
            {
                ...
                g.DrawRectangle(Pens.Black, ServerTank.position.X + 1, ServerTank.position.Y, 10, 10);
                g.FillRectangle(Brushes.Black, ServerTank.position.X + 1, ServerTank.position.Y, 10, 10);
                ServerTank.position = new Point(x + 1 + PanForce, y);
                ServerTank.cPosition = new Coordinate(x + 1 + PanForce, y);  //nuut
    ...
    class Server
    {
        ...
        public void User(Socket client)
        {
            while (true)
            {
                byte[] msg = new byte[2048*15];   //1024  //2048 2048 * 2 = 4kilo bytes WAS *7
